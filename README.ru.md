...
- `download --decrypt` скачивает каждый пак *и* сразу его расшифровывает;
  расшифрованный вывод группируется в `decrypted/<server>/`, чтобы несколько
  загрузок не смешивались. Без флага инструмент выводит команду `decrypt --all`,
